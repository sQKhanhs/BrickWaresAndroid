-- Feedback: a category the user picks in the dialog, stored so feedback can be sorted (2026-09-16).
-- 'bug' | 'feature' | 'other' (default 'other' — covers old rows and any caller that omits it).
alter table public.feedback
    add column category text not null default 'other'
    check (category in ('bug', 'feature', 'other'));

-- Redefine submit_feedback with a p_category argument. Adding a parameter would create a second
-- overload, so drop the previous signature first, then recreate with the new arg (default 'other',
-- coerced to 'other' if a client sends anything unexpected). Body is otherwise migration
-- 20260916120000 (signed-in account email) unchanged.
drop function if exists public.submit_feedback(text, text, text, text, text, text, text);
