...
// Hosts the DEV flavor may reach over plain http — the local Supabase stack (emulator alias, loopback,
// and the LAN IP from BRICKWARES_DEV_SUPABASE_URL when set). Everything else, in every variant, is
// https-only: the network security config below replaces the old blanket usesCleartextTraffic="true".
val devCleartextHosts: List<String> = buildList {
    addAll(listOf("10.0.2.2", "127.0.0.1", "localhost"))
    // `java.net.URI` must be imported: inside a build script a bare `java.` resolves to the Gradle java extension.
    runCatching { URI(devSupabaseUrl) }.getOrNull()
        ?.takeIf { it.scheme == "http" && !it.host.isNullOrBlank() }
        ?.let { if (it.host !in this) add(it.host) }
}

/**
 * Writes `res/xml/network_security_config.xml` for one variant: cleartext OFF everywhere, plus an
 * allow-list of plain-http hosts for the dev flavor. Generated (not a static file) so the LAN host for
 * physical-device testing comes from local.properties instead of being committed.
 */
abstract class GenerateNetworkSecurityConfig : DefaultTask() {
    ...
    fun generate() {
        ...
        val allowList = if (hosts.isEmpty()) "" else buildString {
            append("\n    <!-- Dev flavor only: the local Supabase stack is plain http. -->\n")
            append("    <domain-config cleartextTrafficPermitted=\"true\">\n")
            hosts.forEach { append("        <domain includeSubdomains=\"false\">$it</domain>\n") }
            append("    </domain-config>")
        }
        val xml = """
            |<?xml version="1.0" encoding="utf-8"?>
            |<!-- GENERATED by app/build.gradle.kts (GenerateNetworkSecurityConfig) — do not edit by hand. -->
            |<network-security-config>
            |    <!-- Explicit for API 26–27 too (28+ already defaults to no cleartext). -->
            |    <base-config cleartextTrafficPermitted="false" />$allowList
            |</network-security-config>
            |""".trimMargin()
        val xmlDir = outputDir.get().asFile.resolve("xml").apply { mkdirs() }
        xmlDir.resolve("network_security_config.xml").writeText(xml)
    }
}
...
androidComponents {
    onVariants { variant ->
        // Per-variant network security config (see GenerateNetworkSecurityConfig above).
        val hosts = if (variant.flavorName == "dev") devCleartextHosts else emptyList()
        val taskName = "generate${variant.name.replaceFirstChar { it.uppercase() }}NetworkSecurityConfig"
        val task = project.tasks.register<GenerateNetworkSecurityConfig>(taskName) {
            cleartextHosts.set(hosts)
            outputDir.set(project.layout.buildDirectory.dir("generated/res/networkSecurityConfig/${variant.name}"))
        }
        variant.sources.res?.addGeneratedSourceDirectory(task, GenerateNetworkSecurityConfig::outputDir)

        // prodDebug gets its own application id so a debuggable build talking to the PRODUCTION
        // backend can never be installed over — or mistaken for — the Play package. Only prodDebug:
        // the dev flavor is already ".dev", and changing devDebug would only churn the daily
        // emulator workflow (new package = reinstall + lost local data) for no security gain.
        // ⚠️ Consequences (one-time setup): Firebase needs an Android app for
        // com.senniapp.brickwares.debug in google-services.json (the google-services plugin fails the
        // prodDebug build otherwise), and Google sign-in needs an Android OAuth client for that
        // package + the DEBUG SHA-1 (handoff §0ay).
        if (variant.flavorName == "prod" && variant.buildType == "debug") {
            variant.applicationId.set("com.senniapp.brickwares.debug")
        }
    }
}
